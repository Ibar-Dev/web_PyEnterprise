    ...
    return rx.card(
        rx.vstack(
            ...
            rx.cond(
                ...
                rx.spinner(),
                rx.cond(
                    EmployeeDashboardState.proyectos.length() > 0,
                    rx.text("Proyectos cargados desde Supabase aparecerían aquí"),
                    rx.text("No tienes proyectos asignados actualmente",
                           color=COLORS["text_light"], text_align="center")
                )
            ),
            spacing="4",
            align="stretch",
    ...
    return rx.card(
        rx.vstack(
            ...
            rx.cond(
                ...
                rx.spinner(),
                rx.cond(
                    EmployeeDashboardState.tareas.length() > 0,
                    rx.text("Tareas cargadas desde Supabase aparecerían aquí"),
                    rx.text("No tienes tareas asignadas",
                           color=COLORS["text_light"], text_align="center")
                )
            ),
            ...
            rx.card(
                rx.vstack(
                    ...
                    rx.cond(
                        ...
                        rx.spinner(),
                        rx.cond(
                            EmployeeDashboardState.jornadas.length() > 0,
                            rx.text("Jornadas cargadas desde Supabase aparecerían aquí"),
                            rx.text("No hay jornadas registradas",
                                   color=COLORS["text_light"], text_align="center")
                        )
                    ),
